...
class Toutiao:
    ...
    def searchPicture(self, scrollTime:int):
        self.browser.get('https://so.toutiao.com/search?dvpf=pc&source=input&keyword={}&pd=atlas&action_type=search_subtab_switch&page_num=0&from=gallery&cur_tab_title=gallery'.format(self.keyword))
    ...
    def __searchMicro(self, page:int):
        self.browser.get('https://so.toutiao.com/search?dvpf=pc&source=input&keyword={}&pd=weitoutiao&action_type=search_subtab_switch&page_num={}&from=weitoutiao&cur_tab_title=weitoutiao'.format(self.keyword, page))
        time.sleep(5)
        ...
        for i in range(len(urls)):
            ...
            for pic_attr in pics_attrs:
                ...
                temp = self.__divideToutiao(source="今日头条微头条", author=author, title=title,
                                            ...
                                            url_article=self.browser.current_url, url_pic=pic_url,
    ...
    def __searchNews(self, page:int):
        self.browser.get('https://so.toutiao.com/search?dvpf=pc&source=input&keyword={}&pd=information&action_type=search_subtab_switch&page_num={}&from=news&cur_tab_title=news'.format(self.keyword, page))
        time.sleep(5)
        ...
        for i in range(len(urls)):
            ...
            pics_attrs = self.browser.find_element(By.CLASS_NAME, "article-content").find_elements(By.TAG_NAME, "img")
            for pic_attr in pics_attrs:
                pic_url = pic_attr.get_attribute("src")
